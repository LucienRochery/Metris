...
EdgeSeed::EdgeSeed(MeshBase& msh,
    ...
    const auto lnoed = tdim_ent == 1 ? lnoed1 : tdim_ent == 2 ? lnoed2 : lnoed3;
    ...
    // Dimension of the entity holding the edge, read before the collapse balls
    // widen the cavity below.
    int shell_dim = -1;
    ...
    if (icollapse) {
        ...
        // Collapse compatibility. Two ends both constrained to an entity the edge does
        // not lie in are not adjacent there, so merging them tears it. One end alone
        // being lesser dim is fine: the other is simply removed onto it.
        if (shell_dim > MAX(pdim0, pdim1)) {
            ierro = INS2D_ERR_COLEDGDIM;
            return;
        }
        // A collapse removes both ends, so their balls are cavity seeding, not growth.
        for (int ii = 0; ii < 2; ii++)
            ball(msh, ipedg[ii], cav.lcedg, cav.lcfac, cav.lctet, &iopen, true, ithrd);
    }

    // Read from the seeded cavity. A ball reaches no lower than its point's own
    // dimension, so for a collapse this is MIN(pdim0, pdim1): the dimension the
    // merged point inherits, whether or not the ends share one.
    tdimp = -1;
    if (cav.lcedg.get_n() > 0)
        tdimp = 1;
    else if (cav.lcfac.get_n() > 0)
    ...
    bool ithrow = !(iseed >= 0 && iseed < msh.nentt(tdimp)) ||
                  isdeadent(iseed, msh.ent2poi(tdimp)) || iref < 0;
    if (ithrow)
        METRIS_THROW_MSG(
