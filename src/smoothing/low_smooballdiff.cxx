...
int smooballdiff(Mesh<MFT>& msh, int ipoin, 
  ...
  auto quafun = get_quafun<MFT,gdim,tdim>(iquaf);
  auto d_quafun = get_d_quafun<MFT,gdim,tdim>(iquaf);
  ...
  *qmax0 = -1.0e30;
  ...
  for(int niter1 = 0; niter1 < miter1; niter1++){
    ...
    while(true){
      ...
      if(!fpreset){
        ...
        if(abs(fpre - fcur) < ftol * abs(fpre)){
          if(iverb >= 3) printf("   - Relative decrease %15.7e < %15.7e end\n",
      ...
      if(ierro > 0){
        if(iverb >= 2) printf("optim_newton_drivertype error %d\n",ierro);
        goto cleanup;
      }
      if(iflag <= 0) {
      ...
      for(int ientt : lball){
        if constexpr (ideg == 1){
          getmeasentP1<idim,idim>(ent2poi[ientt], msh.coord, 
                                  Defaults::vtol, NULL, &iinva);
        }else{
      ...
      for(int iball = 0; iball < nball && !iinva; iball++){
        ...
        bool iflat;
        getmeasentP1<idim,idim>(ent2poi[ient2], msh.coord, 
                                Defaults::vtol, NULL, &iflat);
        if(iflat){
          fcur = 1.0e10;
          break;
        }
        ...
        if(iverb >= 6){
          printf("       - %d: elem %d grad = ",iball,ient2);
          dblAr1(idim,dqelt).print();
          if(ihess){
            printf("       - hess = ");
            dblAr1(nhess,hqelt).print();
          }
        }
        if(niter == 1 && niter1 == 0){
          *qnrm0 += quael; 
    ...
    if(iverb >= 5) writeMesh("debug_smooth0.meshb",msh);

    ierro = msh.interpMetBack(ipoin, idim, lball[0], -1, NULL);
    if(ierro > 0){
      if(iverb >= 2) printf("   # interpMetBack failure ierro = %d \n",ierro);
      //printf(" ipoin = %d \n",ipoin);
    ...
    for(int iball = 0; iball < nball; iball++){
      int ient2 = lball[iball];
      bool iflat;
      getmeasentP1<idim,idim>(ent2poi[ient2], msh.coord, Defaults::vtol, NULL, &iflat);
      if(iflat){
        if(iverb >= 3) printf("    - Flat iball %d element %d \n",iball,ient2);
        goto cleanup;
      }
    }
    ...
    *qmax1 = -1.0e30;
    ...
    if(iverb >= 3) printf("     - Newton update initial quality avg %15.7e " 
                          "max %15.7e \n",*qnrm0,*qmax0);
    if(iverb >= 3) printf("     -                 final quality avg %15.7e " 
                          "max %15.7e \n",*qnrm1,*qmax1);
  }
  ...
  if(*qnrm1 > *qnrm0){
    ierro = 2;
    if(iverb >= 3) printf("   - Local smoo reject: quality norm increase "
               "%f -> %f \n", *qnrm0, *qnrm1);
    goto cleanup;
  }
  ...
  if(iinva){
    if(iverb >= 3) printf(" - End reject validity\n");
    goto cleanup;
  }
  ...
  *qmax1 = *qmax0;
  ...
  if(msh.param->dbgfull){
    if constexpr (ideg >= 2){
      ...
      for(int ientt : lball){
        bool iflat;
        double meas0 = getmeasentP1<idim,idim>(ent2poi[ientt], msh.coord, Defaults::vtol, NULL, &iflat);
